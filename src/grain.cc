	...
	if(r>0 && s>0) {
        ...
        if(pipe_formula) {
            ...
            if(p_master->d<S->H_z){
                ...
                p_master->d = 4. * pow((2. * S->mu_0 * s) / ( M_PI * r), 1./3);
                p_master->l = s;
            }
            else{
                //new formulas for a fracture
                p_master->d = (128 * s * S->mu_0) / (M_PI * r);
                p_master->l = s;

                //p_master->d = (8. / M_PI) * sqrt(2 * s * S->mu_0 / r);  (*old bug*)
                //p_master->l = (1 / 8.) * sqrt(r * s / (2 * S->mu_0));   (*old bug*)
            }
        }
